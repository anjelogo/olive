...
import {
  upsertCustomData,
  getCustomData,
} from "../../../main/internals/CustomDataHandler";
import {
  LogChannelTypes,
  LoggingModuleData,
} from "../../../../Database/interfaces/LoggingModuleData";
...
export default class Log extends Command {
  ...
  private typesAvailable = async (
    ...
    const customData = getCustomData(
      ...
      interaction instanceof CommandInteraction
        ? interaction.id
        : (interaction.message.interactionMetadata?.id as string)
    )?.data as CustomDataStructure;
  ...
  private createContainer = (
    ...
    const customData = getCustomData(bot, interactionID)
        ?.data as CustomDataStructure,
      container: ContainerComponent = {
        ...
        components: [
          ...
          {
            type: Constants.ComponentTypes.TEXT_DISPLAY,
            content: customData.types.length
              ? customData.types.map((t) => `\`${t}\``).join(", ")
              : "None",
          },
          ...actionRow,
        ],
          ...
          components: [
            {
              ...
              label: "Add Log",
              disabled: !(await this.typesAvailable(this.bot, interaction))
                .length,
            ...
            },
            {
              ...
              label: "Remove Log",
              disabled:
                this.validTypes.filter((t) => !customData.types.includes(t))
                  .length === this.validTypes.length,
          ...
          components: [
            {
              ...
              maxValues: (await this.typesAvailable(this.bot, interaction))
                .length,
              minValues: 1,
              options: (await this.typesAvailable(this.bot, interaction)).map(
                (t) => ({ label: t, value: t })
              ),
            },
          ],
  ...
  readonly execute = async (
    ...
    const guild = this.bot.findGuild(interaction.guildID) as Guild,
      data = await this.bot.getModuleData("Logging", {
        guildID: guild.id,
    ...
    if (!data) {
      return interaction.createFollowup({
        content: `${this.bot.constants.emojis.x} Logging module is not setup on this guild`,
      });
    }

    switch (subcommand) {
      case "modify": {
        const channelData = data.channels.find(
      ...
      case "delete": {
        ...
        if (!channelData)
          return interaction.createFollowup({
            content: `${this.bot.constants.emojis.x} This channel is not a log channel`,
          });

        data.channels.splice(data.channels.indexOf(channelData), 1);

        try {
          await this.bot.updateModuleData("Logging", data, {
            guildID: guild.id,
          });

          return interaction.createFollowup({
            content: `${this.bot.constants.emojis.tick} Removed log channel`,
          });
        } catch (e) {
          await interaction.createMessage({
            content: `${this.bot.constants.emojis.x} An error occured while removing the log channel`,
          });
  ...
  readonly update = async (
    ...
    const guild = this.bot.findGuild(component.guildID) as Guild,
      customData = (await getCustomData(
        this.bot,
        component.message.interactionMetadata?.id as string
      )?.data) as CustomDataStructure,
      moduleData = (await this.bot.getModuleData(
        "Logging",
        { guildID: guild.id }
      )) as LoggingModuleData;
    ...
    switch (component.data.customID.split("_")[2]) {
        ...
        return component.editOriginal({
          components: [
            this.createContainer(
              this.bot,
              component,
              (await this.actionRow(this.bot, component)).addLog
            ),
          ],
        });
      }
        ...
        return component.editOriginal({
          components: [
            this.createContainer(
              this.bot,
              component,
              (await this.actionRow(this.bot, component)).deleteLog
            ),
          ],
        });
      }
      case "save": {
        ...
        if (
          moduleData.channels.find((c) => c.channelID === component.channelID)
        )
          moduleData.channels.splice(
            moduleData.channels.findIndex(
              (c) => c.channelID === component.channelID
            ),
            1
          );
        ...
        try {
          await this.bot.updateModuleData("Logging", moduleData, {
            guildID: guild.id,
          });
      ...
      case "addlogtype": {
        customData.types = [
          ...customData.types,
          ...((
            component.data as MessageComponentSelectMenuInteractionData
          ).values.getStrings() as LogChannelTypes[]),
        ];
        return await component.editOriginal({
          components: [
            this.createContainer(
              ...
              component,
              (
                await this.actionRow(this.bot, component)
              ).home
            ),
          ],
        });
      ...
      case "removelogtype": {
        customData.types = customData.types.filter(
          (t) =>
            !(
              component.data as MessageComponentSelectMenuInteractionData
            ).values
