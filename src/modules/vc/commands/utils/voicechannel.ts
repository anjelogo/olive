import { Embed, Guild, GuildChannel, Member, Role, VoiceChannel } from "eris";
...
export default class Voicechannel extends Command {
	...
	public execute = async (interaction: ApplicationCommandManager): Promise<ApplicationCommandManager | FollowupManager | undefined> => {
		...
		case "set": {
			...
			case "name": {
				if (!member.voiceState.channelID)
					return interaction.deny("You need to be in a Private Voice channel to run this command!");
				...
				if (!channel || !cat)	
					return interaction.deny("Could not find channel.");
		
				const channelObj: Channel | undefined = cat.channels.find((c) => c.channelID === channel.id);
				if (!channelObj)
					return interaction.deny("That Voice Channel is not a Private Voice Channel!");
				if (channelObj.owner !== member.id) return interaction.deny("You're not the owner of this voice channel!");
				...
				try {
					...
					return interaction.reply(`${this.constants.emojis.tick} Successfully changed the name of the channel to \`${suboptionvalue}\``);
				} catch (e) {
					return interaction.reply(`${this.constants.emojis.warning.red} Error trying to edit the channel name! Perhaps insufficient permissions?`);
				}
			}
			...
			case "owner": {
				...
				if (!newOwner || !channel.voiceMembers.map((m) => m.id).includes(newOwner.id))
					return interaction.deny("The new owner needs to be in the voicechannel!");
				...
				try {
					...
					return interaction.reply(`${this.constants.emojis.tick} Successfully transferred ownership of Private Channel to \`${newOwner.username}\``);
				} catch (e) {
					return interaction.reply(`${this.constants.emojis.warning.red} Error trying to edit the channel owner!`);
				}
			}
			...
			case "category": {
				...
				if (channel.type !== 4)
					return interaction.deny("That channel is not a category!");

				const categories: Category[] = data.categories;

				if (categories.map((c) => c.catID).includes(channel.id))
					return interaction.deny("That category already exists as a Private Voice Channel Category!");
				...
				try {
					await this.bot.updateModuleData("VC", data, guild);
					await interaction.reply(`${this.constants.emojis.tick} Successfully made \`${channel.name}\` a Private VC category. You can manually rename \`${voice.name}\` to change the name.`);
					return interaction.followup(`${this.bot.constants.emojis.warning.yellow} To remove the channel, simply delete the channel.`, { hidden: true });
				} catch (e) {
					return interaction.reply(`${this.constants.emojis.warning.red} Error trying to add category!`);
				}
			}
		...
		case "lock": {
			...
			if (channelObj.locked)
				return interaction.deny("This channel is already locked!");
			...
			try {
				...
				await channel.editPermission((this.bot.findRole(guild, "@everyone") as Role).id, 35652096, 1048576, "role");

				await this.bot.updateModuleData("VC", data, guild);
				return interaction.reply(`${this.bot.constants.emojis.tick} Locked channel!`, { hidden: true });
			} catch (e) {
				return interaction.reply(`${this.constants.emojis.warning.red} Error trying to lock channel!`);
			}
		}
		...
		case "unlock": {
			...
			try {
				...
				await channel.editPermission((this.bot.findRole(guild, "@everyone") as Role).id, 0, 0, "role");

				await this.bot.updateModuleData("VC", data, guild);
				return interaction.reply(`${this.bot.constants.emojis.tick} Unlocked channel!`, { hidden: true });
			} catch (e) {
				return interaction.reply(`${this.constants.emojis.warning.red} Error trying to unlock channel!`);
			}
		}

		case "information": {
			const selectedChannel = subcommand.options
				? (subcommand.options as ApplicationCommandOption[])[0].value as string
				: member.voiceState.channelID ?? undefined;

			if (!selectedChannel)
				return interaction.deny("Specify or join a private voice channel to the information of a channel!");
			...
			if (!channel)
				return interaction.deny("Not a valid channel!");

			const cat: Category | undefined = data.categories.find((c: Category) => c.catID === channel.parentID);

			if (!cat)
				return interaction.deny("That's not a Private Voice Channel!");
