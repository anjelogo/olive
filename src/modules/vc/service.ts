  ...
  protected fields: InputField[] = [
    {
      ...
      description:
        "Set the default name for new private voice channels created by the bot.",
      type: "list_input",
  ...
  > {
    return {
      "/": async (req, res) => {
        ...
        const currentData = (await this.bot.getModuleData("VC", {
          guildID,
        })) as VCModuleData;
      ...
      "/defaultchannelname": async (req, res) => {
        ...
        if (
          !(await this.bot
            .getModule("Main")
            .hasPerm(req.user as User, "vc.edit.name", guildID))
        ) {
          res.status(403).json({
          ...
          case "POST": {
            try {
              ...
              if (!Array.isArray(bodyData.channel)) {
                res
                  .status(400)
                  .json({ message: "Invalid data format for default name" });
                return;
              }

              currentData.defaultName.channel = bodyData.channel;
              await this.updateData(
                { module: "VC", ctx: { guildID } },
                currentData
              );
